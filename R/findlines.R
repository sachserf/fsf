...
#'
#' @description Works like grep or which for a bunch of files (line by line). Useful example: 1. Always use the same predefined word(s) to mark positions in scripts where you should review something at some time. 2. Search for those words within a bunch of files to get an overview.
#'
#' @param expression Character. Specify expression to match the line.
#' @param exclusive Logical. If TRUE use which, else grep.
#' @param path Character. Specify file path.
#' @param pattern Character. Specify file pattern.
